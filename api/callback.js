...
function renderResult(status, content) {
  ...
  return `<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>IXL Korea CMS Authentication</title></head>
<body>
...
(function () {
  ...
  function send(message) {
    ...
    // Decap replies to the initial "authorizing:github" handshake. Using the
    // received origin keeps the access token scoped to the CMS parent window.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!expectedState || !returnedState || expectedState !== returnedState) {
      throw new Error('OAuth state validation failed. Please start the login again.');
    }
    ...
    res.statusCode = 200;
    res.end(renderResult('success', {
      token: tokenData.access_token,
      provider: 'github'
    }));
  } catch (error) {
    res.statusCode = 200;
    res.end(renderResult('error', {
