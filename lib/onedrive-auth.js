...
const accessTokenInflight = new Map();

function getTokenExpiry(data) {
  const expiresInSeconds = Number(data?.expires_in || 3600);
  const lifetimeMs = Math.max(60, expiresInSeconds) * 1000;
  return Date.now() + Math.max(1000, lifetimeMs - ACCESS_TOKEN_SAFETY_WINDOW_MS);
}

function getCachedAccessToken(cacheKey) {
  const cached = accessTokenCache.get(cacheKey);
  if (!cached) return '';
  if (!cached.token || Number(cached.expiresAt || 0) <= Date.now()) {
...
function setCachedAccessToken(cacheKey, data) {
  const token = String(data?.access_token || '').trim();
  if (!cacheKey || !token) return token;
  accessTokenCache.set(cacheKey, {
    token,
    expiresAt: getTokenExpiry(data)
  });
  return token;
}

function personalAccessTokenCacheKey(connection, refreshToken) {
  const tokenHash = crypto
    .createHash('sha256')
    .update(String(refreshToken || ''))
    .digest('hex')
    .slice(0, 24);
  return `personal:${connection.id}:${tokenHash}`;
}

function businessAccessTokenCacheKey(connection) {
  return `business:${connection.id}:${connection.tenantId}:${connection.clientId}`;
}

async function getOrCreateAccessToken(cacheKey, factory) {
  const cached = getCachedAccessToken(cacheKey);
  if (cached) return cached;

  if (accessTokenInflight.has(cacheKey)) {
    return accessTokenInflight.get(cacheKey);
  }

  const pending = (async () => {
    const data = await factory();
    return setCachedAccessToken(cacheKey, data);
  })();

  accessTokenInflight.set(cacheKey, pending);
...
async function refreshPersonalAccessToken(connection, req, res) {
  ...
  const cacheKey = personalAccessTokenCacheKey(connection, refreshToken);
  ...
  return getOrCreateAccessToken(cacheKey, async () => {
    ...
    if (data.refresh_token && data.refresh_token !== refreshToken) {
      ...
      const rotatedKey = personalAccessTokenCacheKey(connection, data.refresh_token);
      setCachedAccessToken(rotatedKey, data);
    }
...
async function getBusinessAccessToken(connection) {
  const cacheKey = businessAccessTokenCacheKey(connection);
